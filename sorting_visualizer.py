...
def generate():
    ...
    for bar in range(1, 60):
        ...
        bar = canvas.create_rectangle(
            barstart, randomY, barend, 365, fill='orange')
        barList.append(bar)
        barstart += 10
        barend += 10
...
window = tk.Tk()
window.title('Sorting Visualizer -Gopikrishna')
...
# Buttons
insert = tk.Button(window, text='Insertion Sort',
                   command=insertion_sort, width=10, height=1)
select = tk.Button(window, text='Selection Sort',
                   command=selection_sort, width=10, height=1)
bubble = tk.Button(window, text='Bubble Sort',
                   command=bubble_sort, width=10, height=1)
